...
export function Rating({ review }: RatingProps) {
  ...
  async function handleAddBadRating() {
    if (session.status === 'authenticated') {
      ...
      if (data.status === 'success') {
        ...
        toast({
          title: 'Você nao gostou do produto',
          description: 'Avaliação (GL) adicionada com sucesso',
        })
      }
    }
  }
  ...
  return (
    <div className="space-x-2">
      <Button
        className="bg-green-500 hover:bg-green-600 text-zinc-50 space-x-1"
        onClick={handleAddGoodRating}
      >
        <span>Gostei</span> <Smiley size={20} weight="fill" />
      </Button>

      <Button
        className="bg-red-500 hover:bg-red-600 text-zinc-50 space-x-1"
        onClick={handleAddBadRating}
      >
